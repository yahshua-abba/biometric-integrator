<template>
  <div>
    <header class="bg-slate-900 text-white p-6 space-y-4">
      ...
      <p class="text-slate-300">Synthetic employees only. This is the app’s actual Retry Queue page, connected to a local test Payroll.</p>
      <ol class="list-decimal ml-5 text-sm space-y-1 text-slate-300"><li>Run ordinary sync: employee mapping failures and missing confirmations enter the queue.</li><li>Fix employee mappings, then run ordinary sync again: the failed records stay in the queue.</li><li>Filter a date and employee, select records, and retry. Watch mini Payroll below.</li></ol>
...
const controls = [['sync', 'Run ordinary sync'], ['fix', 'Fix employee mappings'], ['delete', 'HR deletes saved logs'], ['reset', 'Reset demo']]
async function load() {
